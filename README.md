...
In this part of the repository we go over the whole process of creation of a Data warehouse (DW). Our objective is to build a minimum viable product capable of storing not only occurrance data of each species, but also their main carachteristics along with the climatic data of the regions they have been observed in (PostgreSQL and Pentaho were used in this project). This effectively unifies all the important information required for most of the analyis on animal sighttings. Only geographical occurrences and biodiversity data on carnivore mammals is used, but the DW can be easily extended. 
